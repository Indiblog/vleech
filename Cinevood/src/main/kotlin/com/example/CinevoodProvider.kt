...
// https://github.com/rockhero1234/cinephile/blob/master/Cinevood/src/main/kotlin/com/cinevood/Cinevood.kt
// Changes:
//   - package: com.cinephile.cinevood → com.example
//   - mainUrl: hardcoded string → DomainConfig.CINEVOOD  (ONLY domain change)
//   - class name: Cinevood → CinevoodProvider (matches Plugin registration)
//   - All scraping logic, data classes, and extractors are IDENTICAL to the original.
...
class CinevoodProvider : MainAPI() {
    // ► Fallback domain; overridden at runtime by DomainResolver if domains.json has a newer value.
    override var mainUrl = DomainConfig.CINEVOOD
    ...
    private suspend fun ensureDomain() {
        ...
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            mainUrl = DomainResolver.resolveBlocking("CINEVOOD", DomainConfig.CINEVOOD)
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (isSingle) {
            ...
            val episodes = transposed.map { episodeData ->
                val epLinks  = episodeData.map { it.second }
                val epTitle  = episodeData.firstOrNull()?.first ?: "Untitled"
                newEpisode(epLinks.toJson()) { this.name = epTitle }
            }
    ...
    override suspend fun loadLinks(
        ...
        val links = tryParseJson<List<String>>(data)?.filterNot { it.isBlank() }
            ?: data.trim().removePrefix("[").removeSuffix("]")
                .split(',')
                .mapNotNull { raw ->
    ...
    private fun getSearchQuality(check: String?): SearchQuality? {
        val s = check?.lowercase() ?: return null
        return when {
            s.contains("webrip") || s.contains("web-dl") -> SearchQuality.WebRip
            s.contains("bluray")                          -> SearchQuality.BlueRay
            s.contains("hdts") || s.contains("hdcam") || s.contains("hdtc") -> SearchQuality.HdCam
            s.contains("dvd")                             -> SearchQuality.DVD
            s.contains("camrip") || s.contains("rip")    -> SearchQuality.CamRip
            s.contains("cam")                             -> SearchQuality.Cam
            s.contains("hdrip") || s.contains("hdtv")    -> SearchQuality.HD
