...
// https://github.com/rockhero1234/cinephile/blob/master/Cinevood/src/main/kotlin/com/cinevood/Extractor.kt
// Changes: package com.cinevood → com.example. All logic is identical.
...
class HubCloud : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        if (href.isBlank()) { Log.w("HubCloud", "No valid href found"); return }
        ...
        document.select("div.card-body h2 a.btn").amap { element ->
            ...
            val linkBase = getBaseUrl(link)

            when {
                text.contains("FSL Server", ignoreCase = true) -> {
                    callback(newExtractorLink("$referer [FSL Server]", "$referer [FSL Server] $labelExtras", link) { this.quality = quality })
                }
                text.contains("Download File", ignoreCase = true) -> {
                    callback(newExtractorLink("$referer", "$referer $labelExtras", link) { this.quality = quality })
                }
                text.contains("BuzzServer", ignoreCase = true) -> {
                    ...
                    if (dlink.isNotBlank()) {
                        callback(newExtractorLink("$referer [BuzzServer]", "$referer [BuzzServer] $labelExtras", linkBase + dlink) { this.quality = quality })
                ...
                text.contains("pixeldra", ignoreCase = true) || text.contains("pixel", ignoreCase = true) -> {
                    ...
                    else "$linkBase/api/file/${link.substringAfterLast("/")}?download"
                ...
                text.contains("S3 Server", ignoreCase = true) -> {
                    callback(newExtractorLink("$referer S3 Server", "$referer S3 Server $labelExtras", link) { this.quality = quality })
                }
                text.contains("10Gbps", ignoreCase = true) -> {
                    ...
                    while (true) {
                        ...
                        if (redirectUrl == null) { Log.e("HubCloud", "10Gbps: No redirect"); return@amap }
                        if ("link=" in redirectUrl) break
                        currentLink = redirectUrl
                    }
                    val finalLink = redirectUrl!!.substringAfter("link=")
                    callback(newExtractorLink("$referer 10Gbps [Download]", "$referer 10Gbps [Download] $labelExtras", finalLink) { this.quality = quality })
                }
    ...
    private fun cleanTitle(title: String): String {
        ...
        val startIndex   = parts.indexOfFirst { p -> qualityTags.any { p.contains(it, ignoreCase = true) } }
        val endIndex     = parts.indexOfLast  { p ->
            subTags.any { p.contains(it, ignoreCase = true) } ||
            audioTags.any { p.contains(it, ignoreCase = true) } ||
            codecTags.any { p.contains(it, ignoreCase = true) }
        }
        return when {
